...
const works = [
  ...
  { slug: 'portfolio-2', caseKey: 'vietravel-report', title: 'VIETRAVEL', type: 'Business report · Presentation design', role: 'Information design · Art direction', cardDescription: 'A 54-slide business report shaped into one visual system.', description: 'A visual presentation system for a 2022 business review and 2023 action plan.', challenge: 'Dense tables, targets and operating plans needed a clear, consistent presentation language.', outcome: 'A 54-slide deck spanning performance, strategy, FIT, GIT and a 12-month plan. All business data is fictionalized for confidentiality.', tags: ['Presentation design', 'Information design'], image: vietravelCover, deep: true },
  ...
  { slug: 'portfolio-6', caseKey: 'echo', title: 'ECHO', type: 'Brand identity / Blockchain ticketing', year: '2026', role: 'Product strategy / Identity / UX/UI', cardDescription: 'One smart ticket from transfer rules to venue entry.', description: 'A blockchain ticketing and culture-pass concept for independent venues.', challenge: 'Ticket ownership, transfer limits and gate status often feel fragmented or opaque.', outcome: 'One visual and product system connecting issue, receive, transfer, entry and post-event attendance.', tags: ['Blockchain UX', 'Brand identity'], image: echoHero, deep: true },
];

const legacyWorkRoutes = [
  { from: 'mat', to: 'portfolio-2' },
...
const heroCarouselImages = [
  ...
  { image: echoHero, alt: 'ECHO blockchain ticketing identity and product concept' },
...
function ProjectPage({ work }) {
  ...
  return <><a className="skip-link" href="#main-content">Skip to content</a><Nav /><main id="main-content" className="project-page">
    <section className="project-hero section-pad">
      <div className="page-shell"><Reveal><Link className="project-back" to="/#portfolio"><ArrowLeft size={16} /> Portfolio</Link><div className="project-heading"><div><span className="eyebrow">{work.type}</span><h1>{withoutTrailingPeriod(work.title)}</h1></div><p>{work.description}</p></div></Reveal><Reveal className="project-cover" delay={.08}><img src={work.image} alt={`${work.title} case study cover`} /></Reveal></div>
    </section>
